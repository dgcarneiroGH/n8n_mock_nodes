...
try {
  ...
  const DATE_FIELDS = [
    ["property_fecha_de_recepci_n", "receptionDate"],
    ["property_fecha_de_inicio_de_convocatoria", "startDate"],
    ["property_fecha_de_fin_de_convocatoria", "endDate"],
  ];
  ...
  function mergeDates(item) {
    ...
    const merged = { ...item };
    for (const [sourceKey, targetKey] of DATE_FIELDS) {
      const value = notionGrant[sourceKey]?.start;
      if (value) merged[targetKey] = value;
    }
    return merged;
  }
